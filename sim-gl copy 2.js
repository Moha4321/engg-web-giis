...
const pixelRatio  = Math.min(window.devicePixelRatio, isMobile ? 1 : isLowEnd ? 1.5 : 2);

// Particle counts scaled by device
const WIND_COUNT   = isMobile ? 80  : isTablet ? 200 : 400;
const SPARK_COUNT  = isMobile ? 20  : isTablet ? 50  : 100;
...
const skidMat = new THREE.MeshStandardMaterial({
  color: 0x050505, roughness: 1, transparent: true, opacity: 0, depthWrite: false
});
...
const tyreGeo = new THREE.CylinderGeometry(0.52, 0.52, 0.55, 20);
const tyreMat = new THREE.MeshStandardMaterial({ color: 0x0d0d0d, roughness: 0.95, metalness: 0.05 });
...
for (let col = 0; col < TYRE_COLS; col++) {
  for (let row = 0; row < TYRE_ROWS; row++) {
    const tyre = new THREE.Mesh(tyreGeo, tyreMat.clone());
...
const barrierMat = new THREE.MeshStandardMaterial({ color: 0x2a2a2e, roughness: 0.9, metalness: 0.15 });
...
const DEBRIS_COUNT = isLowEnd ? 30 : 80;
...
const FLASH_COUNT = isLowEnd ? 15 : 40;
...
const FIRE_COUNT = isLowEnd ? 30 : 80;
...
function animate(now) {
  ...
  lastTime  = now;
  const raw = Math.min(ms / 1000, 0.033); // cap at 33ms (30fps minimum)
  accumulator += raw;

  resizeRenderer();

  // Fixed physics steps — max 2 per frame to keep mobile smooth
  let steps = 0;
  while (accumulator >= SIM_DT && steps < 2) {
